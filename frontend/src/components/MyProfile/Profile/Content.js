...
export default function Content(props) {
  ...
  const upvote = async (e, id) => {
    if (localStorage.getItem("username") !== null) {
      ...
      const response = await fetch(
        `http://localhost:8000/api/answer/upvote/${id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const downvote = async (e, id) => {
    if (localStorage.getItem("username") !== null) {
      ...
      const response = await fetch(
        `http://localhost:8000/api/answer/downvote/${id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchVotes = async () => {
    const response = await fetch(
      `http://localhost:8000/api/answer/fetchVotes`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const acceptAnswer = async (e, id) => {
    const response = await fetch(
      `http://localhost:8000/api/answer/acceptanswer/${id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
                      ...
                      if (ans.status === "Accepted") {
                        ...
                        return (
                          <>
                            <button
                              className="btn btn-white"
                              onClick={(e) => acceptAnswer(e, ans._id)}
                            >
                              <i
                                class="fa fa-check"
